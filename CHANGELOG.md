...
- Discord-only sites (email disabled, the default since Monitoring 7.7.0) now record a delivered Discord alert in the notified-issue state and apply the automatic 24-hour cooldown. Before this change, these sites reposted the same alert to Discord on every run and never sent a recovery notice, because that state was only saved after an email was delivered. Sites that send email keep the email-based behavior.
- A recovery confirmed in the same run as a new alert is now included in the Discord alert as a `Recovery | <component>` field. Before this change, it was cleared from the state without being shown in Discord.
...
### Monitoring 7.7.0 - 2026-09-16

- Email delivery is now disabled by default. Existing site configurations automatically add `"EnableEmailNotifications": false` after the monitor updates, and email can be enabled per site by changing the value to `true`.
- Discord delivery remains independent. Test or daily email commands are skipped cleanly while email is disabled, without blocking normal checks, logs, or eligible Discord alerts.
...
- The monitor now honors `options.instanceName` as the authoritative SQL named instance. Local and remote SQL targets are constructed directly from that setting, then the configured database is selected and tested with `SELECT 1`; unrelated instances are no longer probed.
- Configurations without `options.instanceName` retain the legacy SQL-instance discovery fallback.
...
- Kept the database-selection screen limited to database names and retained the manual connection workflow as a fallback. Stored passwords are still decrypted only in memory; legacy plaintext configurations are supported without displaying the password.
