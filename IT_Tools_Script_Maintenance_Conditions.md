...
- Use `D4A-ScheduledMonitor.ps1` as the canonical monitoring file for every new deployment. Preserve each deployed monitor filename because production files may use names such as `D4A-ScheduledMonitor-v5.ps1`, `D4A-ScheduledMonitor-v6.ps1`, or `D4A-ScheduledMonitor-v7.1.2.ps1`.
...
- Keep the current major release at `7.0`. Future feature releases use `7.1.0`, `7.2.0`, and so on. Same-day minor corrections may use a patch version such as `7.2.1`. Do not change the major version to `8.0` or above without an explicit user request.
...
- Keep Monitoring release metadata separate from IT Tools: `version.txt` is only for IT Tools, while `monitor-version.txt` and the matching monitor definition in `update-manifest.json` must agree with the monitored script's embedded version, release date, filename, and SHA-256 before installation. New deployments use `canonicalMonitoring` and `D4A-ScheduledMonitor.ps1`; legacy versioned monitors use `monitoring` and `D4A-ScheduledMonitor-v5.ps1` as an update bridge. Keep both definitions and payloads synchronized until legacy production monitors are no longer supported. Retry temporary GitHub/CDN disagreement with visible run-log progress and a finite limit.
...
- Persist notification-eligible issues for recovery tracking only after at least one enabled channel, email or Discord, confirms successful delivery. Create the automatic cooldown after that successful delivery. Send one recovery notification only after the same check explicitly returns `OK`, and clear its saved recovery state only after at least one enabled channel confirms delivery; never infer recovery merely because a check or result is missing.
