...
- Keep Monitoring release metadata separate from IT Tools: `version.txt` is only for IT Tools, while `monitor-version.txt` and the `monitoring` definition in `update-manifest.json` must agree with the monitored script's embedded version, release date, filename, and SHA-256 before installation. Retry temporary GitHub/CDN disagreement with visible run-log progress and a finite limit.
